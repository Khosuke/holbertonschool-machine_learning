...
class Decision_Tree():
    ...
    def possible_thresholds(self, node, feature):
        """
        Return the possible split thresholds for a feature at a given node.
        Args:
            node: Node whose samples are considered.
            feature: Index of the feature to evaluate.
        Returns:
            A NumPy array containing the midpoints between consecutive
    ...
    def Gini_split_criterion_one_feature(self, node, feature):
        """
        Find the threshold with the lowest Gini impurity for one feature.

        Args:
            node: Node being split.
            feature: Index of the feature being evaluated.

        Returns:
            The best threshold and its corresponding Gini impurity.
        """
        # Target values of the samples in the node
        targets = self.target[node.sub_population]

        # Feature values of the samples in the node
        values = self.explanatory[:, feature][node.sub_population]

        # Different classes present in this node
        # Shape: (c,)
        ...
        # True when sample n belongs to class c
        # Shape: (n, c)
        is_class = (targets[:, None] == classes[None, :])

        # True when sample n goes to the left for threshold t
        # Shape: (n, t)
        left_samples = (values[:, None] <= thresholds[None, :])

        # left_F[n, t, c] → sample n is class c AND goes left for threshold t
        left_F = (left_samples[:, :, None] & is_class[:, None, :])

        # True when sample n goes to the right for threshold t
        # Shape: (n, t)
        right_samples = (values[:, None] > thresholds[None, :])

        # right_F[n, t, c] → sample n is class c AND goes right for threshold t
        right_F = (right_samples[:, :, None] & is_class[:, None, :])

        # Number of samples of each class for each threshold
        # Shape: (t, c)
        left_counts = left_F.sum(axis=0)
        right_counts = right_F.sum(axis=0)

        # Number of samples in the left and right child for each threshold
        # Shape: (t,)
        left_size = left_counts.sum(axis=1)
        right_size = right_counts.sum(axis=1)

        # Proportion of each class in each child
        # Shape: (t, c)
        left_proportions = left_counts / left_size[:, None]
        right_proportions = right_counts / right_size[:, None]

        # Gini impurity for each possible threshold
        # Shape: (t,)
        left_gini = 1 - np.sum(left_proportions ** 2, axis=1)
        right_gini = 1 - np.sum(right_proportions ** 2, axis=1)

        # Total number of samples for each threshold
        total = left_size + right_size

        # Weighted average of the left and right Gini impurities
        # Shape: (t,)
        gini_average = (
            (left_size / total) * left_gini + (right_size / total) * right_gini
        )

        # Index of the threshold with the lowest Gini impurity
        best_index = np.argmin(gini_average)

        # Best threshold and its Gini impurity
        best_threshold = thresholds[best_index]
        best_gini = gini_average[best_index]

        return best_threshold, best_gini

    def Gini_split_criterion(self, node):
        """
        Find the best feature and threshold to split a node, based on
        the lowest weighted Gini impurity across all features.
        Args:
            node: Node being split.
        Returns:
            A tuple (feature_index, threshold)
        """
        X = np.array(
